...
int Attestation::verify_evidence(buffer_t *evidence, const uint8_t* pubkey) {
    ...
    if (memcmp(claim->value, enclave_signer_id, OE_SIGNER_ID_SIZE) != 0) {
        TRACE_ENCLAVE("Signer id does not match the one provided - did you compile the client with the correct 'enclave_signing_key.h' file?");
        goto exit;
    }
...
// Phenotype::Phenotype(std::string _message, ComputeServerMessageType _mtype) 
